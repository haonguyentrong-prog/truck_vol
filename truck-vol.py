...
if uploaded_file:
    try:
        ...
        if len(range_1) == 2 and len(range_2) == 2:
            ...
            # SỬA LỖI Ở ĐÂY: Dùng map thay cho applymap cho các phiên bản Pandas mới
            if hasattr(display_df.style, 'map'):
                st.dataframe(display_df.style.map(style_remark, subset=['Truck Remark', 'Vol Remark']), use_container_width=True, hide_index=True)
            else:
